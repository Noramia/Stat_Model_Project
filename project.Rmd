---
title: "Statistical Modelling III Project"
...
Holding height constant, the full model predicts that an increase of of 1kg will on average increase the length of the catheter by 0.42081cm. 
...
The aim of this study is to produce the best predictive model for mammographic mass severity. The model must be parsimonious whilst having minimun residual error. Predicted probabilities of severity may also be generated from the model. This information may 
...
```{r, fig.cap = "Comparison Matrix depicting the relationship between Margin, Shape, Severity and Age.", warning = F, echo = F}
ggpairs(na.omit(dplyr::select(mammo, Age, Shape, Margin,Severity)))
```

From figures 8 and 9 we can make the following observations:

Mild positive correlation is observed between Age and the other variables. There correlations are 0.364 and 0.411 for Shape and Age and Margin and Age respectively.
...
```{r, fig.cap = "Boxplot of Age Against Density", echo = F, warning = F}
...
This model was obtained by first starting with a saturated model with all the two way interaction terms. Statistically insiginificant terms were removed via the backwards selection algorithmn at the 5% significance level. Hence the final model is the most parsimonious model with all the statistically significant predictors. 
...
Shape: Holding all other variables constant, having a lesion of Shape = 2 (oval) increases the log-odds of having a malignant tumour by -.4478 compared to Shape = 1 (round). For Shape = 3 (lobular), th increase is 0.4992 and for Shape = 4 (irregular), the increase is 1.2428, all relative to Shape = 1.
...
```{r, fig.cap = "Boxplot of Age Against Shaoe", eval = F, warning = F}
plot(mammo$Age~mammo$Shape, xlab = "Shape", ylab = "Age")
```
